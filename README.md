...
- **Transport & headers** — a **strict Content-Security-Policy** (`default-src 'self'`, no `unsafe-inline`; `object-src`/`frame-src`/`frame-ancestors 'none'`; `connect-src`/`form-action`/`base-uri 'self'`), `X-Frame-Options: DENY`, `nosniff`, `Referrer-Policy: no-referrer`, Permissions-Policy, **COOP + CORP `same-origin`**, and `X-Permitted-Cross-Domain-Policies: none` on every response; HSTS outside dev; **`Cache-Control: no-store` on every HTML response** (so authenticated vault pages are never written to the browser's disk cache or served from the back button after sign-out, while static assets and images keep their own caching); `SameSite=Strict` + `HttpOnly` + `Secure` on all cookies; global `AutoValidateAntiforgeryToken`; per-IP fixed-window rate limiting on the auth surface (returns `429`); default-deny fallback authorization policy; no `Server` header. Because the CSP forbids inline scripts and styles, **all** interaction is wired through `site.js` data-attributes (no inline `onclick`/`style` anywhere) — a CSP-clean pattern rather than a `'unsafe-inline'` escape hatch.
